...
extern "C"
{
  ...
  const char * /*unsigned long long*/ open_pipe_server(const char *prefix,
                                      const char *client,
                                      int show_client);
  const char *open_pipe_client(const char *name);
  const char *read_from_pipe(const char */*unsigned long long*/ hPipe);
  void write_to_pipe(const char * /*unsigned long long*/ hPipe, const char *s);
